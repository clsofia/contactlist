import React, { useContext, useState } from "react";
import{useNavigate, useParams} from "react-router";
import { Context } from "../store/appContext";





export const CardComponent = props => {

const{actions} = useContext(Context)

const navigate = useNavigate()

const [showConfirm, setShowConfirm] = useState(false);

const[contactDelete, setContactDelete] = useState(null)



const HandleDeleteClick = (contact) => {
  setContactDelete(contact);
  setShowConfirm(true);
};

const HandleDeleteConfirmClick = () => {
  actions.deleteUser(contactDelete.id);
  setShowConfirm(false);
};

const HandleCancelClick = () => {
  setShowConfirm(false);
};

return(
<div className="card"style={{ width: '18rem' }}>

<div className="modal" tabIndex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="mi-modal" style={{ display: showConfirm ? "inline-block" : "none" }}>
  <div className="modal-dialog modal-lg">
    <div className="modal-content">
      <div className="modal-header">
        <h4 className="modal-title" id="myModalLabel"> Are you sure you want to delete { contactDelete && contactDelete.full_name}</h4>
        </div>
        <div className="modal-body">
        <h9 className="modal-comment">If you delete this thing, the entire universe will go down!</h9>
      </div>
      <div className="modal-footer">
        <button type="button" className="btn btn-default" id="modal-btn-si" onClick={e => HandleDeleteConfirmClick()}>Yes Baby!</button>
        <button type="button" className="btn btn-primary" id="modal-btn-no" onClick={HandleCancelClick}>Oh No!</button>
      </div>
    </div>
  </div>
</div>

<div className="card container-fluid d-flex align-items-center" style={{ width: '60rem' }}>
    <div className="container row no-gutters row d-flex justify-content-around px-0">
        <div className="col-md-8">
            <div className="contact">
              
              <div className="new-contact">
                <div className="row">
                  <div className="col-md-2 col-sm-2">
                    <img src="data:image/jpeg;base64," alt="Image" height="100" width="100" />
                  </div>
                  <div className="col-md-7 col-sm-7">
                  <h5 className="card-title">{props.el.full_name}</h5>
                  <p className="card-text">{props.el.address}</p>
                  <p className="card-text">{props.el.phone}</p>
                  <p className="card-text">{props.el.email}</p>
                  </div>
                  <div className="col-md-3 col-sm-3 pull right">
                    <button className="btn btn-primary btn-sm" onClick={e => navigate("/edit/" + props.el.id)}>Edit</button>
                    <button className="btn btn-danger btn-sm" onClick={e=> HandleDeleteClick(props.el)}>Delete</button>
                  </div>
                </div>
             </div>
           </div>
         </div>
     </div>
     </div>
    



</div>

)

}
